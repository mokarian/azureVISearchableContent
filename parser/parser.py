...
class Parser:
    ...
    def parse_vi_json(self, json):
        """
        This method parses JSON file (created by VI) and distribute each Item
        under videos[0]["insights"] to different intervals having fixed duration (e.g. 10000 milliseconds interval)
        :param json:
        ...
        for video in json["videos"]:
            ...
            if "namedLocations" in insights:
                intervals = self.parse_named_locations(insights["namedLocations"], intervals)
            if "namedPeople" in insights:
                intervals = self.parse_named_people(insights["namedPeople"], intervals)
            if "audioEffects" in insights:
                intervals = self.parse_audio_effects(insights["audioEffects"], intervals)
            ...
            if "visualContentModeration" in insights:
                intervals = self.parse_visual_content_moderation(insights["visualContentModeration"], intervals)
            if "framePatterns" in insights:
                intervals = self.parse_frame_patterns(insights["framePatterns"], intervals)
    ...
    def create_intervals(self, video, show_name):
        ...
        duration_in_milliseconds = int(self.time_parser.string_time_to_milliseconds(video["insights"]["duration"]))
        for i in range(0, duration_in_milliseconds):
            if i % self.time_parser.interval_in__milliseconds == 0:
                start_time = self.time_parser.seconds_to_time_string(i / 1000)
                end_time = self.time_parser.seconds_to_time_string(
                    duration_in_milliseconds) \
                    if i + self.time_parser.interval_in__milliseconds >= duration_in_milliseconds \
                    else self.time_parser.seconds_to_time_string(
                    (i + self.time_parser.interval_in__milliseconds) / 1000)
                dictionary_of_intervals[i] = \
                    {"id": video_id + "-" + str(int(i / self.time_parser.interval_in__milliseconds)),
    ...
    def insert_items_to_intervals(occurred_intervals, item_tuple, intervals):
        """
        This method inserts occurred intervals into the dictionary of intervals (that covers all  the video)
        :param occurred_intervals:
        :param item_tuple: a tuple of Item . e.g. : ("faces", {"face": face["name"],
                                                       "assets": "{...}"})
        :return:
        """
    ...
    def parse_transcript(self, transcripts, intervals):
        """
        this  method parses transcript and add related data to dictionary of intervals
        :param transcripts:
        :param intervals:
        :return:
        """
        for transcript in transcripts:
            if transcript["text"] != "" and transcript["confidence"] > 0.5:
                for instance in transcript["instances"]:
                    ...
                    assets = {"id": transcript["id"],
                              "speakerId": transcript["speakerId"],
                              "language": transcript["language"],
                              "start": instance["start"],
                              "end": instance["end"]}
                    transcript_object = {"transcript": transcript["text"],
                                         "assets": json.dumps(assets)}

                    tuples = ("transcripts", transcript_object)
                    intervals = self.insert_items_to_intervals(occurred_intervals, tuples,
                                                               intervals)
        return intervals

    def parse_ocr(self, ocrs, intervals):
        """
        this  method parses ocrs and add related data to dictionary of intervals
        :param ocrs:
        :param intervals:
        :return:
        """
        for ocr in ocrs:
            if ocr["text"] != "" and ocr["confidence"] > 0.5:
                for instance in ocr["instances"]:
                    ...
                    occurred_intervals = self.time_parser.get_related_intervals(start,
                                                                                end)
                    assets = {"id": ocr["id"],
                              "left": ocr["left"],
                              "top": ocr["top"],
    ...
    def parse_keywords(self, keywords, intervals):
        """
        this  method parses keywords and add related data to dictionary of intervals
        :param keywords:
        :param intervals:
        :return:
        """
        for keyword in keywords:
            if keyword["text"] != "" and keyword["confidence"] > 0.5:
                for instance in keyword["instances"]:
                    ...
                    assets = {"id": keyword["id"],
                              "language": keyword["language"],
                              "start": instance["start"],
                              "end": instance["end"]}
                    ocr_object = {"keyword": keyword["text"],
                                  "assets": json.dumps(assets)}
    ...
    def parse_topics(self, topics, intervals):
        """
        this  method parses topics and add related data to dictionary of intervals
        :param topics: 
        :param intervals: 
        :return: 
        """
        for topic in topics:
            if topic["name"] != "" and topic["confidence"] > 0.5:
                for instance in topic["instances"]:
                    ...
                    assets = {"id": topic["id"],
                              "referenceId": topic["referenceId"] if "referenceId" in topic else "",
                              "referenceType": topic["referenceType"] if "referenceType" in topic else "",
    ...
    def parse_faces(self, faces, intervals):
        """
        this  method parses faces and add related data to dictionary of intervals
        ...
        for face in faces:
            if face["name"] != "" and face["confidence"] > 0.5:
                for instance in face["instances"]:
                    ...
                    assets = {"id": face["id"],
                              "description": face["description"] if "description" in face else "",
                              "thumbnailId": face["thumbnailId"] if "thumbnailId" in face else "",
                              "knownPersonId": face["knownPersonId"] if "knownPersonId" in face else "",
                              "title": face["title"] if "title" in face else "",
                              "imageUrl": face["imageUrl"] if "imageUrl" in face else "",
                              "thumbnailsIds": instance["thumbnailsIds"] if "thumbnailsIds" in instance else "",
                              "start": instance["start"],
                              "end": instance["end"]}
                    face_object = {"face": face["name"],
                                   "assets": json.dumps(assets)}
                    ...
                    for thumbnail in face["thumbnails"]:
                        ...
                        for instance in thumbnail["instances"]:
                            ...
                            assets = {"id": face["id"],
                                      ...
                                      "start": instance["start"],
                                      "end": instance["end"]}
                            thumbnail_object = {"thumbnail": thumbnail_url,
                                                "assets": json.dumps(assets)}
                            tuples = ("thumbnails", thumbnail_object)
                            intervals = self.insert_items_to_intervals(occurred_intervals,
                                                                       tuples,
                                                                       intervals)
    ...
    def parse_labels(self, labels, intervals):
        """
        this  method parses labels and add related data to dictionary of intervals
        :param labels:
        :param intervals:
        :return:
        """
            ...
            if label["name"] != "":
                for instance in label["instances"]:
                    if instance["confidence"] > 0.5:
                        ...
                        assets = {"id": label["id"],
                                  "referenceId": label["referenceId"] if "referenceId" in label else "",
    ...
    def parse_named_locations(self, named_locations, intervals):
        """
        this  method parses namedLocations and add related data to dictionary of intervals
        ...
        for namedLocation in named_locations:
            if namedLocation["name"] != "" and namedLocation["confidence"] > 0.5:
                for instance in namedLocation["instances"]:
                    ...
                    assets = {"id": namedLocation["id"],
                              "referenceId": namedLocation["referenceId"] if "referenceId" in namedLocation else "",
                              "referenceUrl": namedLocation["referenceUrl"] if "referenceUrl" in namedLocation else "",
                              "description": namedLocation["description"] if "description" in namedLocation else "",
                              "confidence": namedLocation["confidence"] if "confidence" in namedLocation else "",
                              "instanceSource": instance["instanceSource"],
                              "start": instance["start"],
                              "end": instance["end"]}
                    label_object = {"namedLocation": namedLocation["name"],
                                    "assets": json.dumps(assets)}
    ...
    def parse_named_people(self, named_people, intervals):
        """
        this  method parses namedPeople and add related data to dictionary of intervals
        :param named_people:
        :param intervals:
        :return:
        """
        for name in named_people:
            if name["name"] != "" and name["confidence"] > 0.5:
                for instance in name["instances"]:
                    ...
                    assets = {"id": name["id"],
                              "referenceId": name["referenceId"] if "referenceId" in name else "",
                              "referenceUrl": name["referenceUrl"] if "referenceUrl" in name else "",
                              "description": name["description"] if "description" in name else "",
                              "confidence": name["confidence"] if "confidence" in name else "",
                              "instanceSource": instance["instanceSource"],
                              "start": instance["start"],
                              "end": instance["end"]}
                    label_object = {"namedPerson": name["name"],
                                    "assets": json.dumps(assets)}
    ...
    def parse_audio_effects(self, audio_effects, intervals):
        """
        this  method parses audioEffects and add related data to dictionary of intervals
        :param audio_effects:
        :param intervals:
        :return:
        """
        for audioEffect in audio_effects:
            if audioEffect["type"] != "":
                for instance in audioEffect["instances"]:
                    ...
                    assets = {"id": audioEffect["id"],
                              "start": instance["start"],
                              "end": instance["end"]}
                    label_object = {"audioEffect": audioEffect["type"],
                                    "assets": json.dumps(assets)}
    ...
    def parse_sentiments(self, sentiments, intervals):
        """
        this  method parses sentiments and add related data to dictionary of intervals
        :param sentiments:
        :param intervals:
        :return:
        """
        for sentiment in sentiments:
            if sentiment["sentimentType"] != "" and sentiment["averageScore"] > 0.5:
                for instance in sentiment["instances"]:
                    ...
                    assets = {"id": sentiment["id"],
                              "averageScore": sentiment["averageScore"] if "averageScore" in sentiment else "",
                              "start": instance["start"],
                              "end": instance["end"]}
                    label_object = {"sentimentType": sentiment["sentimentType"],
                                    "assets": json.dumps(assets)}
    ...
    def parse_emotions(self, emotions, intervals):
        """
        this  method parses emotions and add related data to dictionary of intervals
        :param emotions:
        :param intervals:
        :return:
        """
            ...
            if emotion["type"] != "":
                for instance in emotion["instances"]:
                    if instance["confidence"] > 0.5:
                        ...
                        assets = {"id": emotion["id"],
                                  "confidence": instance["confidence"] if "confidence" in instance else "",
                                  "start": instance["start"],
                                  "end": instance["end"]}
                        label_object = {"emotion": emotion["type"],
                                        "assets": json.dumps(assets)}
    ...
    def parse_visual_content_moderation(self, visual_contents, intervals):
        """
        this  method parses visual_contents and add related data to dictionary of intervals
        ...
        for content in visual_contents:
            for instance in content["instances"]:
                ...
                assets = {"id": content["id"],
                          "start": instance["start"],
                          "end": instance["end"]}
                label_object = {"adultScore": str(content["adultScore"]),
                                "assets": json.dumps(assets)}
            ...
            for instance in content["instances"]:
                ...
                assets = {"id": content["id"],
                          "start": instance["start"],
                          "end": instance["end"]}
                label_object = {"racyScore": str(content["racyScore"]),
                                "assets": json.dumps(assets)}
    ...
    def parse_frame_patterns(self, frame_patterns, intervals):
        """
        this  method parses framePatterns and add related data to dictionary of intervals
        :param frame_patterns:
        :param intervals:
        :return:
        """
        for pattern in frame_patterns:
            if pattern["patternType"] != "" and pattern["confidence"] > 0.5:
                for instance in pattern["instances"]:
                    start = self.time_parser.string_time_to_milliseconds(instance["start"])
                    end = self.time_parser.string_time_to_milliseconds(instance["end"])
                    occurred_intervals = self.time_parser.get_related_intervals(start,
                                                                                end)
                    assets = {"id": pattern["id"],
                              "confidence": pattern["confidence"],
                              "start": instance["start"],
                              "end": instance["end"]}
                    frame_patterns_object = {"framePattern": pattern["patternType"],
                                             "assets": json.dumps(assets)}
    ...
    def parse_brands(self, brands, intervals):
        """
        this  method parses brands and add related data to dictionary of intervals
        :param brands:
        :param intervals:
        :return:
        """
            ...
            if brand["name"] != "" and brand["confidence"] > 0.5:
                for instance in brand["instances"]:
                    ...
                    assets = {"id": brand["id"],
                              ...
                              "start": instance["start"],
                              "end": instance["end"]}
                    frame_patterns_object = {"brand": brand["name"],
                                             "assets": json.dumps(assets)}
